...
def get_body(html):
    return list(filter(lambda child: child.tag == 'body', html.getchildren()))[0]
...
def is_results_page(url):
    ...
    while not q.empty():
        ...
        css_check = check_css_classes(node)
        ...
        if len(child_blocks) > 0:
            ...
            for child in child_blocks:
                ...
                if child.get('type') not in nodes_to_ignore:
                    q.put(child)

            if valid_block_count >= 2:
                print('res set to true')
                res = True
